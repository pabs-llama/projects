...
if selected_cryptos:
    st.line_chart(pivot_df[selected_cryptos], y_label= "Closing Price (USD)")
...
# Volume Bar Chart
## helps users see trading activity - volume spikes often align with big price moves
...
if selected_cryptos:
    ...
    for symbol in selected_cryptos:
        volume_chart(api_data_df, symbol)

# High vs. Low area Chart
def high_low_area(df, symbol):
    df = df[df["symbol"] == symbol].sort_values("date")
    st.area_chart(df[["high", "low"]].set_index(df["date"]))

if selected_cryptos:
    st.subheader("High vs. Low Area Charts")
    for symbol in selected_cryptos:
        high_low_area(api_data_df, symbol)
...
st.markdown(f"""
## {symbol} Price: **{latest_price:,.2f} EUR**  
...
# Add volume as semi-transparent bars (secondary y-axis)
# fig.add_trace(go.Bar(
#     x=df["date"],
#     y=df["volume"],
#     name="Volume",
...
# ))
## Add a Moving Avg
sma_window = st.selectbox("Moving Average Window", [None, 7, 14, 30], index=0)
...
change_period = st.selectbox("Select Timeframe for Top Movers", ["1D", "1W", "1M", "6M", "1Y"])
